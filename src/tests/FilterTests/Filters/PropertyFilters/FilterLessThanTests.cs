...
//  Author           : RzR
//  Created On       : 2023-10-31 14:20
// 
//  Last Modified By : RzR
//  Last Modified On : 2023-10-31 15:00
// ***********************************************************************
//  <copyright file="FilterLessThanTests.cs" company="">
...
{
    [TestClass]
    public class FilterLessThanTests
    {
        ...
        [DataRow("count", "101", 3)]
        [DataRow("count", "1", 0)]
        [DataRow("date", "2010/05/01", 0)]
        [DataRow("date", "2021/07/01", 3)]
        [DataRow("date", "2111/07/01", 4)]
        [DataRow("endDate", "2111/07/01", 3)]
        [DataRow("endDate", "2011/07/01", 2)]
        public void LessThan_Test(string propertyName, string value, int expected)
        {
            //Act
            var filtered = _fakeItems.PropertyLessThan(propertyName, value);
        ...
        public void NotSupportedContains_Test(string propertyName, string value, int expected)
        {
            ...
            {
                //Act
                Assert.ThrowsException<NotSupportedException>(() => _fakeItems.PropertyLessThan(propertyName, value));
            }
            catch (Exception e)
            {
                //Assert

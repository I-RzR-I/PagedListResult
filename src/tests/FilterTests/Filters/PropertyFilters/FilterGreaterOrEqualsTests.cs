...
//  Author           : RzR
//  Created On       : 2023-10-31 09:01
// 
//  Last Modified By : RzR
//  Last Modified On : 2023-10-31 15:00
// ***********************************************************************
//  <copyright file="FilterGreaterOrEqualsTests.cs" company="">
...
{
    [TestClass]
    public class FilterGreaterOrEqualsTests
    {
        ...
        [DataRow("count", "101", 1)]
        [DataRow("count", "1", 4)]
        [DataRow("date", "2010/05/01", 4)]
        [DataRow("date", "2021/07/01", 1)]
        [DataRow("date", "2111/07/01", 0)]
        [DataRow("endDate", "2111/07/01", 1)]
        [DataRow("endDate", "2011/07/01", 2)]
        [DataRow("endDate", "0001/01/01", 3)]
        public void GreaterOrEquals_Test(string propertyName, string value, int expected)
        {
            //Act
            var filtered = _fakeItems.PropertyGreaterThanOrEquals(propertyName, value);
        ...
        public void NotSupportedContains_Test(string propertyName, string value, int expected)
        {
            ...
            {
                //Act
                Assert.ThrowsException<NotSupportedException>(() => _fakeItems.PropertyGreaterThanOrEquals(propertyName, value));
            }
            catch (Exception e)
            {
                //Assert
